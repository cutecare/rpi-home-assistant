...
RUN pip3 install wheel && pip3 install xmltodict bluepy psutil homeassistant home-assistant-frontend
...
CMD ([ -f /config/configuration.yaml ] && echo "Skip default config" || git clone https://github.com/cutecare/hass-cutecare-config.git /config) && \
   ...
   pip3 install -r /config/home-assistant/homeassistant/package_constraints.txt 2> /dev/null && \
